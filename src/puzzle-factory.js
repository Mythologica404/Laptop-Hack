...
document.addEventListener("lang", () =>
  console.log("LANG CHANGED" + localStorage.getItem("lang"))
);
...
const QUESTIONS = {
  ...
  "text background color": (d) => d.colors["shapetext"],
...
export function generateRandomPuzzle() {
  ...
  const textColor = sample(Object.keys(COLORS));
  const colors = COLORABLE.reduce((obj, color) => {
    obj[color] =
      color == "colortext" || color == "shapetext"
        ? textColor
...
export function generateQuestionAndAnswer(nums, puzzles) {
  ...
  let tempPosTwo;
  do {
    tempPosTwo = randomInt(nums.length);
  ...
  do {
    tempSecondQuestion = sample(Object.keys(QUESTIONS));
  ...
  const question =
    translateQ(firstQuestion) +
    " (" +
    nums[positionOne] +
    ") " +
    andWord +
    " " +
    translateQ(secondQuestion) +
    " (" +
    nums[positionTwo] +
    ")";
