...
export async function doPuzzle() {
  ...
  const metronome =
    puzzleTime == 7
      ? playSound("assets/metronome.mp3")
      : playSound("assets/long-metronome.mp3");
...
async function displayNumbers(numbers) {
  numbers.forEach(
    (n, i) =>
      ($(
        "#square-" + (i + 1)
      ).innerHTML = `<div class="big-numbers can-shrink" id="num-${
        i + 1
      }">${n}</div>`)
  );
...
timeRange.addEventListener(
  "input",
  () => (puzzleTime = $(".time-display").textContent = timeRange.value)
);
puzzleRange.addEventListener(
  "input",
  () =>
    (puzzleAmount = $(".puzzle-display").textContent =
      parseInt(puzzleRange.value))
);
