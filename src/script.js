...
async function start() {
  ...
  // display result
  setInformationText(result ? tr("berhasil ngehack.") : tr("Tet Tot"));
...
function setInformationText(text) {
  ...
  const infoText = `<span class="capital">${capitalized.charAt(
    0
  )}</span>${capitalized.substring(1)}`;
