...
document.addEventListener(
  "lang",
  () => (lang = TRANSLATIONS[localStorage.getItem("lang")])
);
...
const translateQ = (t) =>
  lang.QUESTIONS[TRANSLATIONS.GB.QUESTIONS.indexOf(t)] || t;
...
$("#to-fast").innerText = trMisc("Kecepetan? Latihan lagi ngab");
$("#answer-was").innerText = trMisc("jawabannya");
$("#try-again-button").innerText = trMisc("Coba Hack Lagi");

$("#puzzle-amount").innerText = trMisc("Puzzle amount");
$("#puzzle-time").innerText = trMisc("Puzzle time");
$("#answer-placeholder").setAttribute(
  "placeholder",
  trPuzzle("blue") + " " + trPuzzle("square")
);
...
function updateFlag() {
  ...
  $(".flag-selected-display").classList =
    "flag-selected-display flag-icon-" + lang.toLowerCase();
}
